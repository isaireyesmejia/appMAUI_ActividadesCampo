...
public partial class RegistroActividadesViewModel : ObservableObject
{
    ...
    public RegistroActividadesViewModel(
    ...
    [ObservableProperty]
    private ObservableCollection<ZonaModel> zonas = new();

    [ObservableProperty]
    private ObservableCollection<MunicipioModel> municipios = new();

    [ObservableProperty]
    private ObservableCollection<PredioModel> predios = new();

    [ObservableProperty]
    private ObservableCollection<ActividadListItem> actividades = new();

    [ObservableProperty]
    private ZonaModel? zonaSeleccionada;

    [ObservableProperty]
    private MunicipioModel? municipioSeleccionado;

    [ObservableProperty]
    private PredioModel? predioSeleccionado;

    [ObservableProperty]
    private ActividadListItem? actividadSeleccionada;

    [ObservableProperty]
    private DateTime fechaBusqueda = DateTime.Today;

    [ObservableProperty]
    private bool filtroFechaActivo = true;
    ...
    private async Task CargarZonasAsync()
    {
        ...
        try
        {
            // ObtenerZonasAsync(CancellationToken cancellationToken = default) coincide en firma
            // con Func<CancellationToken, Task<List<ZonaModel>>>, así que se pasa como method group
            // y el token que arma CatalogoCacheService internamente sí llega hasta el HttpClient.
    ...
    private async Task AgregarAsync()
    {
        ...
        await Shell.Current.GoToAsync(
            $"{nameof(RegistroActividadFormPage)}?idPredio={PredioSeleccionado.IntGENPredioKey}&idZona={ZonaSeleccionada.IntGENRegionKey}&idMunicipio={MunicipioSeleccionado.IntGENMunicipioKey}");
    }
    ...
    private async Task EditarAsync()
    {
        ...
        await Shell.Current.GoToAsync(
            $"{nameof(RegistroActividadFormPage)}?idActividad={ActividadSeleccionada.IntAGRRegistroActividadKey}",
            parametros);
    }
